     ...
     var isOff = 0;
     var setup;
     elementArray = ["A1", "A2", "A3", "A4", "A5", "B1", "B2", "B3", "B4", "B5", "C1", "C2", "C3", "C4", "C5", "D1", "D2", "D3", "D4", "D5", "E1", "E2", "E3", "E4", "E5"];
 ...
 test("Ensures game initialization is completely random (More accurate with larger sample size)", () =>
   {
    newGame();
    let firstInit = isOff;
    newGame();
    let secondInit = isOff;
    newGame();
    let thirdInit = isOff;

    expect(firstInit).not.toBe(secondInit);
    expect(secondInit).not.toBe(thirdInit);
    expect(thirdInit).not.toBe(firstInit);
   });
